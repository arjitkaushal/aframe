...
let sub = function (lhs, rhs) {
  if (
    ...
    console.error(
      "only point - point, point - line or line - line subtraction is allowed"
    );
  return {
    x: lhs.x - rhs.x,
    y: lhs.y - rhs.y,
...
let calculateHitMatrix = function (ray_vector, plane_normal, point) {
  ...
  const ray_vector_projection = sub(
    ray_vector,
    mul(plane_normal, dotProduct(ray_vector, plane_normal))
  );
...
function hitTestPlane(frame, ray, plane, frameOfReference) {
  ...
  const plane_normal = transform_point_by_matrix(plane_pose.transform.matrix, {
    x: 0,
    y: 1.0,
    z: 0,
    w: 0,
  ...
  const plane_center = normalize_perspective(
    transform_point_by_matrix(plane_pose.transform.matrix, {
      x: 0,
      y: 0,
      z: 0,
      w: 1.0,
  ...
  if (denominator < 0.0001 && denominator > -0.0001) {
    ...
    if (d < 0) {
      ...
      let point_on_plane = transform_point_by_matrix(
        plane_pose.transform.inverse.matrix,
        point
      ); // hit test point coodinates relative to plane pose
      console.assert(
        Math.abs(point_on_plane.y) < 0.0001,
        "Incorrect Y coordinate of mapped point"
      );
...
// was hit.
export function filterHitTestResults(
  hitTestResults,
  keep_last_plane = false,
  simplify_planes = false
) {
  let result = hitTestResults.filter((hitTestResult) => {
    let polygon = simplify_planes
      ? simplifyPolygon(hitTestResult.plane.polygon)
    ...
    for (let i = 0; i < polygon.length; ++i) {
      ...
      const cosine_ray_segment = crossProduct2d(
        segment_direction,
        turn_direction
      );
      if (side == 0) {
  ...
  if (keep_last_plane && hitTestResults.length > 0) {
    ...
    if (
      last_horizontal_plane_result &&
      result.findIndex((element) => element === last_horizontal_plane_result) ==
        -1
    ) {
